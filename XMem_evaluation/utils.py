...
def plot_verb_chart(verb_class_csv_path, yaml_path, output_path=None, J_dict=None):
    ...
    # ax2.set_ylim((0, 1))
    # ax2.(fontsize=20)
    # ax1.legend(('J_metric'), loc='upper right')
    # ax2.legend(('count'), loc='upper left')
    # plt.bar(range(len(J_list)), J_list, width=0.4, tick_label=caption_list)
    # plt.bar([x+0.4 for x in list(range(len(J_list)))], cnt_list, width=0.4, tick_label=caption_list)
