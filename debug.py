...
# print(np.unique(mask))
frames_orders = torch.stack([torch.cat((torch.tensor([0]),torch.randperm(8-2)+1,torch.tensor([8-1]))) for i in range(2)])
print(frames_orders)
a = torch.rand(2,8,2,2)
print(a[0])
b = torch.stack([a[bi,frames_orders[bi],:,:] for bi in range(2)])
print(b[0])