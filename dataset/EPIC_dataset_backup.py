    ...
    def __getitem__(self, idx):
        ...
        while trials < 5:
            ...
            for f_idx in frames_idx:
                ...
                this_raw_im = Image.open(path.join(vid_im_path, jpg_name)).convert('RGB')
                this_im = self.all_im_dual_transform(this_raw_im)
                ...
                for tmp_idx in range(len(agg_u_frames)):
                    ...
                    # deprecated 改到preprocess里了
                    # 将手的flow去掉
                    # if preprocess_flow is not None:
                    #     this_flowu, this_flowv = self.preprocess_flow.remove_hand_region(np.array(this_raw_im), this_flowu, this_flowv)
                    #     this_backward_u, this_backward_v = self.preprocess_flow.remove_hand_region(np.array(this_raw_im), this_backward_u, this_backward_v)
                    ...
                    if this_flow == None:
                        this_flow = torch.cat([torch.tensor(np.array(this_flowu)).unsqueeze(0), torch.tensor(np.array(this_flowv)).unsqueeze(0)], dim=0)
                    else:
                        this_flow = torch.cat([this_flow, torch.tensor(np.array(this_flowu)).unsqueeze(0), torch.tensor(np.array(this_flowv)).unsqueeze(0)], dim=0)
                    # this_backward_flow 最后的shape是2*L x H x W
                    if this_backward_flow == None:
                        this_backward_flow = torch.cat([torch.tensor(np.array(this_backward_u)).unsqueeze(0), torch.tensor(np.array(this_backward_v)).unsqueeze(0)], dim=0)
                    else:
                        this_backward_flow = torch.cat([this_backward_flow, torch.tensor(np.array(this_backward_u)).unsqueeze(0), torch.tensor(np.array(this_backward_v)).unsqueeze(0)], dim=0)
